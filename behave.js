...
Spec.prototype.evaluate = function (cb) {
  ...
  if (this.async) {
    ...
    for (i = 0, l = this.expectations.length; i < l; i += 1) {
...
Expectation.prototype.evaluate = function () {
  if (this.satisfied) {
    successes += 1;
    log('I expected ' + JSON.stringify(this.someValue) + ' ' + this.comparisonText + ' ' + JSON.stringify(this.otherValue));
  } else {
    failures += 1;
    log('I incorrectly got ' + JSON.stringify(this.someValue) + ', when I expected ' + JSON.stringify(this.otherValue));
  }
};
